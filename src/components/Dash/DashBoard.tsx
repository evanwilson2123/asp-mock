...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [blastRes, hitTraxRes, trackmanRes] = await Promise.all([
        ...
        if (!blastRes.ok || !hitTraxRes.ok || !trackmanRes.ok) {
          throw new Error("Failed to fetch data. Please try again.");
        }

        const [blastJson, hitTraxJson, trackmanJson] = await Promise.all([
          blastRes.json(),
          hitTraxRes.json(),
          trackmanRes.json(),
        ]);
  ...
  // We'll display each "tech" in a separate card, arranged in a grid layout
  return (
      ...
      <div className="flex-1 p-4 text-gray-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded shadow p-4 flex flex-col md:col-span-2 xl:col-span-1">
            ...
            {/* Peak Velocities */}
            <div className="grid grid-cols-2 lg:grid-cols-2 gap-2 mb-2">
              {pitchStats.map((stat) => (
                <div
                  key={stat.pitchType}
