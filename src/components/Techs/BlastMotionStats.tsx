                                        ...
                                        <div className="divide-y divide-gray-200">
                                            {filteredAvailableTags.map((tag) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${selectedTagId === tag._id ? 'bg-blue-50' : ''
                                                        }`}
                                                >
...
const BlastMotionStats: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBlastData = async () => {
            try {
                ...
                console.log(JSON.stringify(sortedSessionAverages));
    ...
    const fastSwingData = sessionData.map((s) => s.fastSwingRates);
    const youthPerc = fastSwingData.map((s) => s.youthPercent);
    const highPerc = fastSwingData.map((s) => s.highschoolPercent);
    const collegePerc = fastSwingData.map((s) => s.collegePercent);
    const proPerc = fastSwingData.map((s) => s.proPercent)

    const fastSwingRateChartData = {
        labels,
        datasets: [
            {
                label: "Youth %",
    ...
    return (
        <div className="flex min-h-screen">
            ...
            <div className="flex-1 p-6 bg-gray-100 flex-col overflow-x-hidden">
                ...
                >
                    <h2 className='text-lg font-semibold text-gray-700 mb-4'>
                        Fast Swing Rates Over Time
                    </h2>
                    {sessionData.length > 0 ? (
                        <div className="w-full h-72 md:h-96">
                            <Line
                                data={fastSwingRateChartData}
                                options={fastChartOptions}
                            />
                        </div>
                    ) : (
                        <p className='text-gray-500'>No Fast Swing Data Available</p>
                    )}
                </div>
